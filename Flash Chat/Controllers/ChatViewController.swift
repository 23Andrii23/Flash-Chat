...
class ChatViewController: UIViewController {
    ...
    @IBAction func sendPressed(_ sender: UIButton) {
        ...
        if let messegeBody = messageTextfield.text, let messageSender = Auth.auth().currentUser?.email {
            db.collection(K.FStore.collectionName).addDocument(data: [K.FStore.senderField: messageSender,K.FStore.bodyField: messegeBody,K.FStore.dateField: Date().timeIntervalSince1970]) { (error) in
                if let e = error {
